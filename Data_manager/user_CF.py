    ...
    def __init__(self, dataMatrix, topK=50, shrink = 0, normalize = True,
        ...
        if similarity == "asymmetric":
            ...
            raise ValueError("Cosine_Similarity: value for parameter 'mode' not recognized."
                             " Allowed values are: 'cosine', asymmetric', 'jaccard', 'tanimoto',"
                             "dice, tversky."
    ...
    def compute_similarity(self, start_col=None, end_col=None, block_size = 100):
        ...
        while start_col_block < end_col_local:
            ...
            if self.use_row_weights:
                ...
                #전체유저 * block 유저 형태 => dot production 사용해 유저간 유사도 측정
            ...
            for col_index_in_block in range(this_block_size):
                ...
                top_k_idx = relevant_items_partition[relevant_items_partition_sorting] #각 컬럼(유저)별 topK유저
...
class BaseUserSimilarityMatrixRecommender(BaseSimilarityMatrixRecommender):
    ...
    def _compute_item_score(self, user_id_array, items_to_compute=None):
        ...
        user_weights_array = self.W_sparse[user_id_array] #유저간 유사도를 필요한 유저만 뽑음
        ...
        return item_scores #특정유저에 대한 아이템 추천 점수가 나올 것.

# In[ ]:


class user_CF(BaseUserSimilarityMatrixRecommender):

    RECOMMENDER_NAME = "user_CF"
    FEATURE_WEIGHTING_VALUES = ["BM25", "TF_IDF"]


    def __init__(self, URM_train, verbose = True):
        super(user_CF, self).__init__(URM_train, verbose = verbose)
    ...
    def fit(self, topK=50, shrink=100, similarity='cosine', normalize=True, feature_weighting = "none", **similarity_args):
        ...
        self.W_sparse = similarity.compute_similarity() #유저간 유사도
