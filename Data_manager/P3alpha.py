...
class P3alpha(BaseItemSimilarityMatrixRecommender):
    """ P3alpha recommender """

    RECOMMENDER_NAME = "P3alpha"

    def __init__(self, URM_train, verbose = True):
        super(P3alpha, self).__init__(URM_train, verbose = verbose)


    def __str__(self):
        return "P3alpha(alpha={}, min_rating={}, topk={}, implicit={}, normalize_similarity={})".format(self.alpha,self.min_rating, self.topK, self.implicit,
                                                                            self.normalize_similarity)

    def fit(self, topK=100, alpha=1.,implicit=False, normalize_similarity=False):
...
#         Pui = self.URM_train
#         Piu = self.URM_train.transpose(copy=True)
